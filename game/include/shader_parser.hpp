...
namespace ShaderParser {
	...
	inline void parseIncludesCPU(std::string& includes, std::string& sourceCode) {
		...
		for (const auto& entry : std::filesystem::directory_iterator(includes)) {
			...
			try {
				...
				Logging::Error(std::format("Includes initialization: file was not successfully read\n{}", entry.path().string()));
			}
	...
	inline void parseIncludesGPU(std::string& includes) {
		for (const auto& entry : std::filesystem::directory_iterator(includes)) {
			...
			glNamedStringARB(GL_SHADER_INCLUDE_ARB, static_cast<unsigned int>(name.size()), name.c_str(), static_cast<unsigned int>(source.size()), source.c_str());
		}
	}
}
