...
void TextRenderer::initRenderer() {
	...
	glBindVertexArray(m_VAO);
	glBindBuffer(GL_ARRAY_BUFFER, m_VBO);
	glBufferData(GL_ARRAY_BUFFER, m_maxBufferVertices * TEXT_VERTIX_COUNT_PER_CHAR * TEXT_VERTIX_DATA_PER_CHAR * sizeof(float), NULL, GL_DYNAMIC_DRAW);
...
void TextRenderer::initFont(std::string_view font, unsigned int fontSize) {
	...
	Logging::Log(std::format("Initializing font: {}", font));
	...
	for (unsigned char c = ASCII_TABLE_START; c <= ASCII_TABLE_END; ++c) {
		if (FT_Load_Char(face, c, FT_LOAD_RENDER)) {
			Logging::Error(std::format("Failed to load Glyph: {}", (char)c));
			continue;
		}
...
void TextRenderer::initFontMSDF(std::string_view atlasPath, std::string_view jsonPath) {
	...
	if (!f.is_open()) {
		Logging::Error(std::format("Could not open the JSON file: {}", jsonPath));
		return;
	}
	json j = json::parse(f);
...
void TextRenderer::render(Shader& shader, std::string_view text, int x, int y, float scale, glm::vec3 color) {
	...
	shader.setMat4("projection", glm::ortho(0.0f, float(Window::getWidth()), 0.0f, float(Window::getHeight()))); // move this to better place
	m_renderText(vertices);
}

void TextRenderer::renderMSDF(Shader& shader, std::string_view text, int x, int y, float scale, glm::vec3 color, bool bold, OutlineData outlineData) {
	std::vector<TextVertex> vertices;
	vertices.reserve(text.size() * 6);
