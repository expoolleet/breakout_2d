...
void Shader::setVec4(std::string_view name, float *vec) {
    glProgramUniform4fv(m_ID, _getUniformLocation(name), 1, vec);
}

void Shader::setVec4(std::string_view name, const glm::vec4 &vec) {
    saveValue(name, vec);
        ...
        std::visit(
            [&](auto &&arg) {
                ...
                if constexpr (std::is_same_v<T, int> || std::is_same_v<T, bool>)
                    glProgramUniform1i(m_ID, location, arg);
                else if constexpr (std::is_same_v<T, float>)
                    glProgramUniform1f(m_ID, location, arg);
                else if constexpr (std::is_same_v<T, glm::vec4>)
                    glProgramUniform4fv(m_ID, location, 1, glm::value_ptr(arg));
                else if constexpr (std::is_same_v<T, glm::vec3>)
                    glProgramUniform3fv(m_ID, location, 1, glm::value_ptr(arg));
                else if constexpr (std::is_same_v<T, glm::vec2>)
                    glProgramUniform2fv(m_ID, location, 1, glm::value_ptr(arg));
                else if constexpr (std::is_same_v<T, glm::mat4>)
                    glProgramUniformMatrix4fv(m_ID, location, 1, GL_FALSE, glm::value_ptr(arg));
                else if constexpr (std::is_same_v<T, glm::mat3>)
                    glProgramUniformMatrix3fv(m_ID, location, 1, GL_FALSE, glm::value_ptr(arg));
            },
            value);
    }
}
