...
template void EventDispatcher::subscribe<BallSpawned>(std::function<void(const BallSpawned &)> callback);
template void EventDispatcher::emit<BallSpawned>(const BallSpawned &eventData);
